...
int main(int argc, char *argv[])
{
	...
	int fileLength;
	fileLength = fread(buffer, 1, maxLength, elfFile); /* 读取文件 */
	buffer[fileLength] = '\0'; 
...
/* 
	检查readelf程序选项
	监测到非法选项会直接返回错误ERR_NOT_EXSIT
	监测到header会开启header标志（将其置1）
	监测到section会开启section标志（将其置1）
	以此类推
 */
STATE_CODE check_readelf_flags(int num_flags, char *argv[])
{
	if (num_flags == 0)
		return ERR_GOOD;
	char elf_header_sign[] = "-h", elf_header_sign_alias1[] = "--file-header"; /* header别名 */
	char elf_section_sign[] = "-S", elf_section_sign_alias1[] = "--section-headers", elf_section_sign_alias2[] = "--sections";/* section别名 */
	char elf_syms_sign[] = "-s", elf_syms_sign_alias1[] = "--syms", elf_syms_sign_alias2[] = "--symbols"; /* symbol别名 */
...
STATE_CODE read32Section(Elf32_Ehdr *header32)
{
	...
	Elf32_Shdr *section_header = (Elf32_Shdr *)((char *)header32 + header32->e_shoff); /* 计算section header表开始地址 */

	char *shstrtab = (char *)header32 + section_header[header32->e_shstrndx].sh_offset; /* 计算section header字符串表开始地址 */
	int num_header = header32->e_shnum; /* 获取section header table数量 */

	for (int i = 0; i < num_header; i++)
	{
		printf("%s", rainbow(i % 7));
		printf("  [%s%2hu%s] %-18s  ",KIRAKIRA, i,KIRADAME, shstrtab + section_header[i].sh_name); /* 打印节头名 */
		getShtype(section_header[i].sh_type); /* 打印节头类型 */
		printf(" %016x   ", section_header[i].sh_addr); /* 打印节头虚拟地址 */
		printf("%08x\n", section_header[i].sh_offset);  /* 打印节头地址偏移 */
		printf("       %016x    %016x  ", section_header[i].sh_size, section_header[i].sh_entsize); /* 打印节头地址偏移 */
		getSectionFlag(section_header[i].sh_flags); /* 打印节头标志 */
		printf("%-4d  %-4d  %-3x \n", section_header[i].sh_link, section_header[i].sh_info, section_header[i].sh_addralign); /* 打印链接地址，附加信息，节对齐信息 */
	}
...
STATE_CODE read64Section(Elf64_Ehdr *header64)
{
	...
	Elf64_Shdr *section_header = (Elf64_Shdr *)((char *)header64 + header64->e_shoff); /* 计算section header表开始地址 */

	char *shstrtab = (char *)header64 + section_header[header64->e_shstrndx].sh_offset; /* 计算section header字符串表开始地址 */
	int num_header = header64->e_shnum; /* 获取section header table数量 */
	...
	for (int i = 0; i < num_header; i++)
	{
		...
		printf("  [%s%2hu%s] %-18s  ", KIRAKIRA,i, KIRADAME,shstrtab + section_header[i].sh_name); /* 打印节头名 */
		getShtype(section_header[i].sh_type); /* 打印节头类型 */
		printf(" %016lx   ", section_header[i].sh_addr); /* 打印节头虚拟地址 */
		printf("%08lx\n", section_header[i].sh_offset); /* 打印节头地址偏移 */
		printf("       %016lx    %016lx  ", section_header[i].sh_size, section_header[i].sh_entsize); /* 打印节头地址偏移 */
		getSectionFlag(section_header[i].sh_flags); /* 打印节头标志 */
		printf("%-4d  %-4d  %-3lx \n", section_header[i].sh_link, section_header[i].sh_info, section_header[i].sh_addralign); /* 打印链接地址，附加信息，节对齐信息 */
	}
...
STATE_CODE read32Symtab(Elf32_Ehdr *header32)
{
	...
	unsigned int num_header = header32->e_shnum;  /* 获取section header table数量 */
	...
	for (int i = 0; i < num_header; i++)
	{
		...
		{
			strtab = (char *)header32 + section_header[i].sh_offset; /* 计算.strtab节表开始地址 */
			break;
		}
	}
	assert(strtab);
	...
	for (int i = 0; i < num_header; i++)
	{
		...
		{
			dynstr = (char *)header32 + section_header[i].sh_offset; /* 计算.dynstr节表开始地址 */
			break;
		}
	}
	assert(dynstr);
	...
	for (int i = 0; i < num_header; i++)
	{
		...
		{
			dynsym = (Elf32_Sym *)((char *)header32 + section_header[i].sh_offset); /* 计算.dynsym节表开始地址 */
			dynsym_shidx = i; /* 记录.dynsym节表索引 */
			break;
		}
	}
	assert(dynsym);
	...
	for (int i = 0; i < num_header; i++)
	{
		...
		{
			symtab = (Elf32_Sym *)((char *)header32 + section_header[i].sh_offset); /* 计算.symtab节表开始地址 */
			symtab_shidx = i; /* 记录.symtab节表索引 */
			break;
		}
	}
	assert(symtab);

	unsigned int dynsym_num_entries = (unsigned int)(section_header[dynsym_shidx].sh_size / section_header[dynsym_shidx].sh_entsize); /* 计算符号表内项数量 */
	...
	for (unsigned int i = 0; i < dynsym_num_entries; i++)
	{
		...
		getStentrytype(ELF32_ST_TYPE(dynsym[i].st_info)); /* 打印entry类型 */
		getStentrybind(ELF32_ST_BIND(dynsym[i].st_info)); /* 打印entry绑定信息 */
		getStentryvis(ELF32_ST_VISIBILITY(dynsym[i].st_other)); /* 打印entry可见性信息 */
		getStentryNdx(dynsym[i].st_shndx); /* 打印entry索引输和特殊索引 */
		printf(" %s", dynstr + dynsym[i].st_name); // Name
		printf("\n");
	}

	unsigned int symtab_num_entries = (unsigned int)(section_header[symtab_shidx].sh_size / section_header[symtab_shidx].sh_entsize); /* 计算符号表内项数量 */
	...
	for (unsigned int i = 0; i < symtab_num_entries; i++)
	{
		...
		getStentrytype(ELF32_ST_TYPE(symtab[i].st_info)); /* 打印entry类型 */
		getStentrybind(ELF32_ST_BIND(symtab[i].st_info)); /* 打印entry绑定信息 */
		getStentryvis(ELF32_ST_VISIBILITY(symtab[i].st_other)); /* 打印entry可见性信息 */
		getStentryNdx(symtab[i].st_shndx); /* 打印entry索引输和特殊索引 */
...
STATE_CODE read64Symtab(Elf64_Ehdr *header64)
{
	...
	unsigned int num_header = header64->e_shnum;  /* 获取section header table数量 */
	...
	for (int i = 0; i < num_header; i++)
	{
		if (strcmp((shstrtab + section_header[i].sh_name), (char *)".strtab") == 0)
		{
			strtab = (char *)header64 + section_header[i].sh_offset; /* 计算.strtab节表开始地址 */
			break;
		}
	}
	assert(strtab);
	...
	for (int i = 0; i < num_header; i++)
	{
		if (strcmp((shstrtab + section_header[i].sh_name), (char *)".dynstr") == 0)
		{
			dynstr = (char *)header64 + section_header[i].sh_offset; /* 计算.dynstr节表开始地址 */
			break;
		}
	}
	assert(dynstr);
	...
	for (int i = 0; i < num_header; i++)
	{
		if (strcmp((shstrtab + section_header[i].sh_name), (char *)".dynsym") == 0)
		{
			dynsym = (Elf64_Sym *)((char *)header64 + section_header[i].sh_offset); /* 计算.dynsym节表开始地址 */
			dynsym_shidx = i;
			break;
		}
	}
	...
	for (int i = 0; i < num_header; i++)
	{
		if (strcmp((shstrtab + section_header[i].sh_name), (char *)".symtab") == 0)
		{
			symtab = (Elf64_Sym *)((char *)header64 + section_header[i].sh_offset); /* 计算.symtab节表开始地址 */
			symtab_shidx = i;
			break;
		}
	}
	...
	for (unsigned int i = 0; i < dynsym_num_entries; i++)
	{
		...
		getStentrytype(ELF64_ST_TYPE(dynsym[i].st_info)); /* 打印entry类型 */
		getStentrybind(ELF64_ST_BIND(dynsym[i].st_info));  /* 打印entry绑定信息 */
		getStentryvis(ELF64_ST_VISIBILITY(dynsym[i].st_other)); /* 打印entry可见性信息 */
		getStentryNdx(dynsym[i].st_shndx); /* 打印entry索引输和特殊索引 */
		printf(" %s", dynstr + dynsym[i].st_name); // Name
		printf("\n");
	}
	...
	for (unsigned int i = 0; i < symtab_num_entries; i++)
	{
		...
		getStentrytype(ELF64_ST_TYPE(symtab[i].st_info));/* 打印entry类型 */
		getStentrybind(ELF64_ST_BIND(symtab[i].st_info));/* 打印entry绑定信息 */
		getStentryvis(ELF64_ST_VISIBILITY(symtab[i].st_other));/* 打印entry可见性信息 */
		getStentryNdx(symtab[i].st_shndx);/* 打印entry索引输和特殊索引 */
		printf(" %s", strtab + symtab[i].st_name); // Name
		printf("\n");
	}
...
fileHeader(char *buffer)
{
	...
	if (procType == ELF_32_BIT)
	{
		...
		if (readelf_flags[READELF_HEADER]) /* 判断elf header打印标志 */
		{
			flag32 = readHeader(header32); 
		}

		if (readelf_flags[READELF_SECTION])  /* 判断section header table打印标志 */
		{
			flag32 = readSection(header32);
		}

		if (readelf_flags[READELF_SYMBOL]) /* 判断 symbol table打印标志 */
		{
